...
def make_tfrec(dataset_dir,tfrec_dir,img_shard):
  ...
  testImages  = sorted(os.listdir(os.path.join(dataset_dir, 'test')))
  trainImages = sorted(os.listdir(os.path.join(dataset_dir, 'train')))
  #random.shuffle(testImages)
  #random.shuffle(trainImages)
  ...
  for i in tqdm(range(num_shards)):    
    ...
    if (i == num_shards-1):
      write_tfrec(write_path, dataset_dir+'/test', testImages[start:])
    else:
      end = start + img_shard
      write_tfrec(write_path, dataset_dir+'/test', testImages[start:end])
      start = end
